...
SET course_students_enrolled = CASE WHEN
 ...
 -- What are the top five "easiest" organizations? Does this have an impact on its rating?
 SELECT
 	course_organization,
    ROUND(SUM(CASE WHEN course_difficulty = 'Beginner' THEN 1 ELSE 0 END) * 100.0 / COUNT(*),2) AS beginner_percentage,
FROM UCoursera_Courses1
GROUP BY course_organization
ORDER BY ROUND(SUM(CASE WHEN course_difficulty = 'Beginner' THEN 1 ELSE 0 END) * 100.0 / COUNT(*),2) DESC, 
LIMIT 5

-- What about the top five hardest?
SELECT
 	course_organization,
    ROUND(SUM(CASE WHEN course_difficulty = 'Advanced' THEN 1 ELSE 0 END) * 100.0 / COUNT(*),2) AS advanced_percentage
FROM UCoursera_Courses1
GROUP BY course_organization
ORDER BY ROUND(SUM(CASE WHEN course_difficulty = 'Advanced' THEN 1 ELSE 0 END) * 100.0 / COUNT(*),2) DESC
LIMIT 5
